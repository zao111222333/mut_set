...
use std::{borrow::Borrow, collections::{ HashMap, HashSet, TryReserveError}, hash::{BuildHasher, Hash, RandomState}, ops::Deref};
    ...
    S: BuildHasher,
{
    ...
    fn extend<I: IntoIterator<Item = T>>(&mut self, iter: I) {
        let vec:Vec<(u64,T::ItemImmutId)> = iter.into_iter().map(|v|(self.hash_one(&v.borrow()),v.into())).collect();
    ...
    T: Item,
{
    ...
    pub fn into_iter(self) -> impl Iterator<Item = T>{
        self.inner.into_iter().map(|(_,v)| <<T as Item>::ItemImmutId as Into::<T>>::into(v) ).into_iter()
    }
    #[inline]
    ...
    T: Item
{
    ...
    pub fn iter(&self) -> impl Clone+Iterator<Item = &T>{
        self.inner.iter().map(|(_,v)| <<T as Item>::ItemImmutId as core::ops::Deref>::deref(&v) ).into_iter()
    }
...
//     }
    ...
    pub fn with_capacity_and_hasher(capacity: usize, hasher: S) -> MutSet<T, S> {
        MutSet { inner: HashMap::with_capacity_and_hasher(capacity, hasher) }
    }
